...
// stream record after it was unmarshalled to a plain object: the ids, the partner, how many
// times the owner has sent the request again and the trace it belongs to. Everything else in the
// image (subject, body, ...) is ignored on purpose: the enqueuer must never handle request text.
export const newRequestImageSchema = z
  ...
  .transform(({ pk, id, partner, retryCount, traceparent }) => ({
    requestId: id,
